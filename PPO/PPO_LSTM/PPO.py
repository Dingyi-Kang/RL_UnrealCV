...
from tensorflow.keras.layers import Conv2D, Flatten, Dense, MaxPooling2D, TimeDistributed, LSTM
import gym
import scipy.signal
import time
...
def preprocess(observation):
    ...
    #so this is in shape of 336x336
    grayscale_observation = np.mean(new_observation, axis=2)
    # Reshape the array to (336, 336, 1)
    reshaped_observation = np.expand_dims(grayscale_observation, axis=-1)
...
def create_cnn(observation_dimensions, num_actions):
    # Input is a 3D tensor with shape (4, inpSize, inpSize)
    input_tensor = keras.Input(shape=observation_dimensions)

    # We use TimeDistributed to apply the same CNN and Flatten layers for each frame
    x = TimeDistributed(Conv2D(filters=32, kernel_size=(5, 5), strides=1, padding='same', activation='relu'))(input_tensor)
    x = TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides=2))(x)
    x = TimeDistributed(Conv2D(filters=32, kernel_size=(5, 5), strides=1, padding='same', activation='relu'))(x)
    x = TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides=2))(x)
    x = TimeDistributed(Conv2D(filters=64, kernel_size=(4, 4), strides=1, padding='same', activation='relu'))(x)
    x = TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides=2))(x)
    x = TimeDistributed(Conv2D(filters=64, kernel_size=(3, 3), strides=1, padding='same', activation='relu'))(x)
    x = TimeDistributed(MaxPooling2D(pool_size=(2, 2), strides=2))(x) 
    #output shape for each temporal slice is (21, 21, 64) -- 28224

    # Added Dense layer to compact the output dimension # but in seed paper, they don't have this step
