...
def create_cnn(observation_dimensions, num_actions):
    ...
    # We use Conv3D to apply the same CNN layers for each frame
    x = Conv3D(filters=32, kernel_size=(1, 5, 5), strides=(1, 1, 1), padding='same', activation='relu')(input_tensor)
    x = MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2))(x)
    x = Conv3D(filters=32, kernel_size=(1, 5, 5), strides=(1, 1, 1), padding='same', activation='relu')(x)
    x = MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2))(x)
    x = Conv3D(filters=64, kernel_size=(1, 4, 4), strides=(1, 1, 1), padding='same', activation='relu')(x)
    x = MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2))(x)
    x = Conv3D(filters=64, kernel_size=(1, 3, 3), strides=(1, 1, 1), padding='same', activation='relu')(x)
    x = MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2))(x) 
    ...
    x = keras.layers.Reshape((-1, np.prod(x.shape[1:])))(x)
...
steps_per_epoch = 250 #store memories of 200 steps
epochs = 1000
gamma = 0.99
clip_ratio = 0.2
...
for epoch in range(epochs):
    ...
    #observation_buffer.shape -- (250, 4, 336, 336, 1)
    #this whole batch cannot be directly passed to ConvLayer since the first CNN layer will generate a tensor of (250, 32, 4, 336, 336) where 32 is num of filters
